...
void LennardJones::calculateForces(System &system)
{
    ...
    for (int current_index = 0; current_index < system.num_atoms(); current_index++) {
        ...
        for (int other_index = current_index+1; other_index < system.num_atoms(); other_index++) {   //to avoid double counting, other_index = current_index+1
            Atom *other_atom = system.atoms(other_index);

            // distance and vector btw objects
            vec2 displacement(0.,0.);
            for (int j = 0; j < 2; j++) {
                displacement[j] = current_atom->position[j] - other_atom->position[j];
            }
            // For 1D domain decomposition, need minimum image convention along y-direction!--> b/c no passing of atoms/ghosts etc there!
            if (displacement[1] >  system.halfsystemSize(1)) displacement[1] -= system.systemSize(1);
            if (displacement[1] <= -system.halfsystemSize(1)) displacement[1] += system.systemSize(1);

            //for case of 1 processor, need to implement x min image convention also
            if(nprocs ==1){
            ...
            double total_force_over_r = 24. * (2.0*pow(radiusSqrd,-7.) - pow(radiusSqrd,-4.)); //Note: attractive force points towards other atom
            for (int j = 0; j < 2; j++) {
        ...
        if (current_atom->position[decomp_dim] < l_skin_bndry) {
            ...
            to_left.push_back(current_atom->position[1]); //note: don't need velocities for ghost atoms
            ...
            if (rank == nprocs-1) {//rightmost proc
                to_right.push_back(current_atom->position[0] - system.systemSize(0));
            } else {
    ...
    */
    ...
    if (nprocs > 1) {
        ...
        //note: these formulas will pass i.e. from proc 0 to proc (max at right)...--> satisfy PBCs...

        // Send ghost atoms to neighboring processor
        MPI_Isend(&num_to_left, 1, MPI_INT, ln, 10*rank + ln, MPI_COMM_WORLD, req);
        ...
        //NOTE: need to only interact with atoms within the skin b/c of cutoff radius
        ...
        for (int current_index=0; current_index<system.num_atoms(); current_index++) {
            ...
            if (current_atom->position[0] < l_skin_bndry) {
                ...
                for (int ghost_index=0; ghost_index < 2*num_from_left; ghost_index+=2) {
                    ...
                    vec2 displacement(0.,0.);
                    for (int j = 0; j < 2; j++) {
                        displacement[j] = current_atom->position[j] - from_left[ghost_index + j];
                    }

                    //Need minimum image convention along y-direction--> b/c no passing of atoms/ghosts etc there
                    //.i.e. think at the corners along skin
                    if (displacement[1] >  system.halfsystemSize(1)) displacement[1] -= system.systemSize(1);
                    if (displacement[1] <= -system.halfsystemSize(1)) displacement[1] += system.systemSize(1);
